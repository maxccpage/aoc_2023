...
const sumPartNumbers = (nums: number[]) =>
...
export default (input: string) => {
  ...
  const getPartNumbers = (base_nums: BaseNumber[]) => {
    return base_nums.reduce<number[]>((acc, curr) => {
      const { values, x, y1, y2 } = curr;

      const all_coords = getCoordRange(x!, y1!, y2!);

      if (isValidPartNumber(all_coords)) {
        const digit = parseInt(values.join(""));
        acc.push(digit);
      }
  ...
  const part_numbers = getPartNumbers(base_numbers);
  const result = sumPartNumbers(part_numbers);
